...
import { A, B, C, D, E, H, L, F} from './constantes.js';

/**
 * Clase para el debug de la Gameboy
 */
class Debug{

    /**
     * Constructor de Debug
    ...
    empezar(){
        ...
        function mostrarRegistrosMemoria(){
            ...
            document.getElementById("regBGP").innerHTML =
                gameboy.regLCD.leerPaletaBGVentana().toString(16).padStart(2, '0');
            document.getElementById("regOBP0").innerHTML =
                gameboy.regLCD.leerPaletaObj0().toString(16).padStart(2, '0');
            document.getElementById("regOBP1").innerHTML =
                gameboy.regLCD.leerPaletaObj1().toString(16).padStart(2, '0');
            const last0 = gameboy.regLCD.debugLastObp0.toString(16).padStart(2, '0');
            const last1 = gameboy.regLCD.debugLastObp1.toString(16).padStart(2, '0');
            document.getElementById("regOBP0W").innerHTML =
                `${last0} (${gameboy.regLCD.debugObp0Writes})`;
            document.getElementById("regOBP1W").innerHTML =
                `${last1} (${gameboy.regLCD.debugObp1Writes})`;
            document.getElementById("regOBP0M").innerHTML =
                `[${gameboy.regLCD.paletaObj0.join(',')}]`;
            document.getElementById("regOBP1M").innerHTML =
                `[${gameboy.regLCD.paletaObj1.join(',')}]`;

            const ds = gameboy.pantalla?.debugSprite;
            if (ds) {
                document.getElementById("regOBJ0").innerHTML =
                    `x${ds.x} y${ds.y} t${ds.tile} r${ds.row} ${ds.low.toString(16).padStart(2,'0')}/${ds.high.toString(16).padStart(2,'0')} p${ds.pal}`;
                document.getElementById("regOBJ0IDX").innerHTML =
                    `[${Array.from(ds.idxs).join(',')}]`;
            } else {
                document.getElementById("regOBJ0").innerHTML = "/";
                document.getElementById("regOBJ0IDX").innerHTML = "/";
            }
